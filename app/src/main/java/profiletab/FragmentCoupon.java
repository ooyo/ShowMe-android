...
public class FragmentCoupon extends Fragment {
    private BonusModel eventModel;
    private List<BonusModel> albumList;
    private RecyclerView recyclerView;
    private BonusAdapter cardAdapter;
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        cardAdapter = new BonusAdapter(container.getContext(), albumList);
    ...
    private void eventHomeList(Context context, final String userid){
        ...
        final String topSlideURL = g.getUserBonus();
            ...
            public void onResponse(String response) {
                try {
                    ...
                    if(jsonResult.getString("resultCode").equals("0")){
                        ...
                        JSONArray bonuslist = results.getJSONArray("bonus");
                        Log.d("JSON", "onResponse: "+results.toString());
                        for(int i=0;i<bonuslist.length(); i++){
                            JSONObject bonusSingle = bonuslist.getJSONObject(i);

                            eventModel = new BonusModel(bonusSingle.getString("bonus_place_id"), bonusSingle.getString("bonus_place"),bonusSingle.getString("bonus_description"),bonusSingle.getString("bonus_end"),bonusSingle.getString("category"));
